...
interface Offer {
  ...
  listing: { businessName?: string; businessImagesUrls?: string[] };
  listingId: string;
  offerPrice?: number;
  offerMessage?: string;
  status: string;
...
export default function OffersPage() {
  ...
  const [offers, setOffers] = useState<Offer[]>([
    {
      ...
      buyer: { id: '4bf237df-5102-4bde-ade2-16fa6bcd149a', name: 'Emmanuel Aboyeji' },
      listing: { businessName: 'Tech Startup', businessImagesUrls: ['/ratel.png'] },
    ...
    },
    {
      ...
      buyer: { id: '5cg348eg-6213-5cef-bef3-27gb7cde250b', name: 'Jane Doe' },
      listing: { businessName: 'E-commerce Store', businessImagesUrls: ['/ratel.png'] },
    ...
    },
    {
      ...
      buyer: { id: '6dh459fh-7324-6dfg-cfg4-38hc8def361c', name: 'John Smith' },
      listing: { businessName: 'Mobile App', businessImagesUrls: ['/ratel.png'] },
    ...
    const fetchOffers = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          const fetchedOffers = response.data.data.offers.map((offer: Offer) => ({
            id: offer.id,
          ...
          setOffers((prev) => fetchedOffers.length ? fetchedOffers : prev);
        } else {
  ...
  const filteredOffers = offers.filter((offer) =>
    activeTab === 'All' ? true : activeTab === 'Pending' ? offer.status === 'PENDING' :
    activeTab === 'Accepted' ? offer.status === 'ACCEPTED' :
    activeTab === 'Rejected' ? offer.status === 'REJECTED' : false
  );
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <p className="text-xs text-gray-600">
                      {offer.status === 'PENDING' ? 'Pending' :
                       ...
                       offer.status === 'REJECTED' ? 'Rejected' : offer.status}
                    </p>
                  </div>
                </div>
              </div>
